...
- **e2e 테스트 `tests/e2e/`** (`pnpm test:e2e`, Playwright + 로컬 wrangler dev). `api` 52건: API 라우트·서버 렌더 페이지의 언어 접두어(`/s/`·`/i/`)·방 웹소켓 프로토콜(조각·완성 정산과 paid, 하루 상한 한 묶음 거절, 재도전 감산, 내 순위, take/mv/grab→deny/drop/untake/lock/merge/resync/release, 나간 사람 점유 풀기). `landing` 16건: 오늘의 퍼즐 창과 폴백. `board` 21건: 판(솔로·회원 조각 XP 묶음·완성 정산·이어하기의 paid·처음부터·294조각의 조각 수 가중치). `room` 9건: 두 브라우저가 조각 꺼내기·되돌리기·나가기. 실행기가 알아서 하는 것: `api`·`landing` 은 배포 빌드, `board`·`room` 은 `NODE_ENV=development astro build --mode development`(훅 `window.__jigsaw` 가 살아 있다 — 이 빌드는 배포하지 말 것. 개발 모드 빌드에서 없는 라우트에 POST 하면 wrangler 프록시가 죽어 `api` 는 배포 빌드로 돈다), `.dev.vars` 가 없으면 더미 비밀값을 만들었다 지운다(회원 흐름은 `authAvailable()` 이 `/api/me` 의 `auth` 를 보므로 비밀값 셋이 있어야 한다), 세션 쿠키 `jl_s` 는 `SESSION_SECRET` 으로 HMAC 해 위조, 테스트 회원(`t000…001`)의 D1 행은 스위트마다 지운다. 훅은 조각을 사람보다 빨리 놓아 완성 정산이 `plausible`(조각당 0.25초)에 걸리므로 `board` 는 조각 수×0.25초를 기다린 뒤 마지막 조각을 놓고, 100조각을 청하면 격자에 맞춰 99조각이 되는 것도 기대값에 반영한다. 헤드리스 크로미움은 en-US 라 컨텍스트를 ko-KR 로 고정한다(안 그러면 언어 자동 이동이 `/en/` 으로 보낸다)
