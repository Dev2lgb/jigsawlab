// 전체 작품 목록 — 기존 22점(lib/jigsaw PAINTINGS + paintings.ts) + AIC 204점(aic.json + worksText + artists)
...
export interface Work { key: string; title: [string, string, string]; artist: [string, string, string]; year: string; cat: string; museum: [string, string, string]; about: [string, string, string]; w: number; h: number; medium?: string; credit?: string; source?: string }
...
export const WORKS: Work[] = [...BASE, ...AIC];
