...
`,
  /** /terms/ 이용약관 */
  terms: `<p>jigsawlab(이하 "사이트")을 이용하면 아래 약관에 동의한 것으로 봅니다. 시행일 2026년 9월 6일.</p>
<h2>1. 서비스</h2><p>사이트는 퍼블릭 도메인 명화와 이용자의 사진으로 직소 퍼즐을 만들어 맞추는 무료 웹 서비스입니다. 가입 없이 쓸 수 있고, 진행 상황·기록·즐겨찾기는 이용자의 기기(브라우저 저장소)에만 보관됩니다. 기기를 바꾸거나 브라우저 데이터를 지우면 함께 사라지며, 사이트는 이를 복구할 수 없습니다.</p>
<h2>2. 이용자의 사진</h2><p>내 사진으로 만든 퍼즐은 브라우저 안에서만 처리되고 서버로 전송되지 않습니다. 이용자는 자신이 권리를 가진 사진만 사용해야 하며, 타인의 저작권·초상권을 침해하는 사진 사용에 대한 책임은 이용자에게 있습니다.</p>
<h2>3. 그림과 콘텐츠</h2><p>사이트의 그림·사진 이미지는 퍼블릭 도메인 또는 CC0로 공개된 작품(시카고 미술관 오픈 액세스, 위키미디어 공용의 퍼블릭 도메인 작품, 미국 의회도서관 포토크롬 컬렉션, NASA·ESA 공개 이미지 등)이며, 각 작품 페이지에 출처와 라이선스를 표시합니다. 그림 소개 글, 디자인, 코드 등 사이트 자체 콘텐츠의 권리는 jigsawlab에 있으며, 개인적·비상업적 범위를 넘는 복제·배포는 사전 동의가 필요합니다.</p>
<h2>4. 금지 사항</h2><p>서비스의 정상 운영을 방해하는 행위, 서버에 과도한 요청을 보내는 행위, 사이트 콘텐츠를 무단 수집·재배포하는 행위는 금지됩니다.</p>
<h2>5. 광고와 제휴</h2><p>사이트에는 광고와 제휴 링크가 실릴 수 있습니다. 제휴 링크를 통한 구매에서 사이트가 수수료를 받을 수 있으며, 제휴 상품의 판매·배송·환불은 해당 판매자의 책임입니다.</p>
<h2>6. 면책</h2><p>서비스는 "있는 그대로" 제공됩니다. 서비스 중단, 데이터 유실, 기기 성능에 따른 문제에 대해 사이트는 법이 허용하는 범위에서 책임을 지지 않습니다.</p>
<h2>7. 약관 변경</h2><p>약관은 필요할 때 바뀔 수 있으며, 바뀐 약관은 이 페이지에 게시한 때부터 적용됩니다.</p>
<h2>문의</h2><p>dev2lgb@gmail.com</p>`,
  /** /privacy/ 개인정보처리방침 — {operator}·{mail} 자리는 Privacy.astro 가 채운다 */
  privacy: `<p>jigsawlab은 가입 없이 쓰는 직소 퍼즐 사이트예요. 개인정보는 최소한만 다룹니다. 시행일 2026년 9월 6일, 최종 개정 2026년 9월 7일.</p>
<h2>사진</h2><p>내 사진으로 퍼즐을 만들 때 사진은 브라우저 안에서만 잘리고 그려집니다. 서버로 전송되거나 저장되지 않습니다. 내 사진 퍼즐에 친구를 초대하면 사진은 WebRTC 로 내 브라우저에서 친구 브라우저로 직접 전달됩니다. 서버는 두 브라우저를 연결하는 신호(연결 주소)만 중계하고 사진 자체는 받지도 저장하지도 않습니다.</p>
<h2>브라우저에 저장되는 것</h2><p>최고 기록, 닉네임, 오늘의 퍼즐 연속 완주 일수가 이 브라우저의 localStorage에만 저장됩니다. 브라우저 데이터를 지우면 함께 사라집니다.</p>
<h2>Google 로그인 (선택)</h2><p>로그인하지 않아도 모든 기능을 쓸 수 있습니다. 완성 기록과 하던 퍼즐을 기기 사이에 이어 가고 싶을 때만 Google 로 로그인하면 됩니다. 로그인 시 Google 에 요청하는 정보는 계정 고유 번호(OpenID sub) 하나뿐이며, 이메일·이름·프로필 사진은 요청하지 않습니다. 고유 번호는 서버 비밀키로 해시한 가명 ID 로만 저장하므로 저장된 값에서 Google 계정을 알아낼 수 없습니다.</p>
<p>로그인하면 서버에 저장되는 것: 가명 ID, 닉네임, 완성 기록(그림 key·조각 수·시간·놓은 횟수·날짜), 하던 퍼즐의 판 상태(명화·오늘의 퍼즐만, 내 사진 퍼즐은 제외). 로그인 세션은 서명된 쿠키(jl_s, 180일)로 유지됩니다. 내 퍼즐 페이지에서 언제든 로그아웃하거나 계정과 서버 기록을 모두 삭제할 수 있으며, 삭제는 즉시 반영됩니다.</p>
<h2>퍼즐 방(같이하기)</h2><p>방을 만들면 그림 key·조각 수·조각 위치·참가자 닉네임이 방이 끝난 뒤 최대 48시간 서버(Cloudflare Durable Objects)에 남았다가 자동 삭제됩니다.</p>
<h2>광고와 제휴 링크</h2><p>광고(Google AdSense)와 제휴 링크가 실릴 수 있습니다. 광고 제공자는 쿠키로 맞춤 광고를 보여줄 수 있으며, 제휴 링크로 구매가 이루어지면 일정 수수료를 받습니다.</p>
<h2>운영 주체와 문의</h2><p>이 사이트는 개인이 운영하는 {operator} 입니다. 위에 적은 정보의 처리 책임자도 {operator} 이고요. 이 방침에 대한 질문, 또는 내 정보의 열람·정정·삭제·처리 거부를 원하시면 {mail} 로 보내 주세요. 로그인한 분은 <b>내 퍼즐</b> 화면에서 계정과 서버 기록 전부를 언제든 직접 지울 수 있습니다.</p>`,
};
