...
export const ES: JigsawStrings = {
  ...
  pick: 'Elegir mi foto', drop: 'Arrastra una foto aquí o toca para elegir', daily: 'Reto de hoy', dailyCap: (title: string, n: number) => `${title} · ${n} piezas`, gallery: 'Elegir rompecabezas', pieces: 'Piezas', custom: 'A medida', gridLabel: (c: number, r: number, n: number) => `${c}×${r} = ${n} piezas`, noImg: 'Elige antes una foto o un cuadro', chosen: 'Imagen elegida', myPhoto: 'Mi foto',
  ...
  time: 'Tiempo', moves: 'Colocadas', left: (n: number) => `quedan ${n} piezas`, edgeOnly: 'Solo los bordes', hint: 'Imagen de fondo', original: 'Ver la imagen completa', zoomIn: 'Acercar', zoomOut: 'Alejar', fit: 'Ajustar', trayHint: 'Arrastra las piezas de la bandeja hacia arriba hasta su sitio. Pellizca para acercar, dos dedos para desplazar',
  viewResult: 'Ver resultado', done: '¡Completo!', resTime: 'Tiempo', resPieces: 'Piezas', resMoves: 'Colocaciones', best: (s: string) => `Tu mejor marca: ${s}`, newBest: '¡Récord nuevo!', retry: 'La misma foto otra vez', another: 'Otro rompecabezas', toTitle: 'Volver al inicio',
