...
export const UI = {
  ko: {
    ...
    nav: { daily: '오늘의 퍼즐', catalog: '퍼즐 목록', photo: '내 사진으로', play: '지금 맞추기' },
    ...
    detail: { open: '이 그림으로 맞추기', pieces: '조각 수를 고르세요', best: (t: string) => `내 최고 기록 ${t}`, noBest: '아직 기록이 없어요', museum: '소장', year: '제작', more: '같은 진열대의 다른 상자', back: '퍼즐 목록으로', gridOf: (c: number, r: number) => `${c}×${r}` },
    guide: '가이드',
  ...
  en: {
    ...
    nav: { daily: "Today's puzzle", catalog: 'Puzzles', photo: 'Your photo', play: 'Play now' },
    ...
    detail: { open: 'Solve this painting', pieces: 'Choose a piece count', best: (t: string) => `Your best ${t}`, noBest: 'No record yet', museum: 'Collection', year: 'Painted', more: 'More from this shelf', back: 'All puzzles', gridOf: (c: number, r: number) => `${c}×${r}` },
    guide: 'Guide',
  ...
  ja: {
    ...
    nav: { daily: '今日のパズル', catalog: 'パズル一覧', photo: '自分の写真で', play: '今すぐ遊ぶ' },
    ...
    detail: { open: 'この絵で遊ぶ', pieces: 'ピース数を選ぶ', best: (t: string) => `自己ベスト ${t}`, noBest: 'まだ記録がありません', museum: '所蔵', year: '制作', more: '同じ棚の他の箱', back: 'パズル一覧へ', gridOf: (c: number, r: number) => `${c}×${r}` },
    guide: 'ガイド',
