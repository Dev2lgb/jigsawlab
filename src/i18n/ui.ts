...
export const UI = {
  ko: {
    ...
    catalog: { title: '퍼즐 상자', sub: '퍼블릭 도메인 명화를 상자처럼 진열했습니다. 상자를 열면 그림 이야기와 조각 수를 고를 수 있어요.', all: (n: number) => `모두 ${n}점`, search: '제목·작가 검색', results: (n: number) => `${n}점`, none: '검색 결과가 없어요', fav: '즐겨찾기', favAdd: '즐겨찾기에 추가', favRemove: '즐겨찾기에서 빼기' },
    ...
    detail: { open: '이 그림으로 맞추기', pieces: '조각 수를 고르세요', best: (t: string) => `내 최고 기록 ${t}`, noBest: '아직 기록이 없어요', museum: '소장', year: '제작', medium: '재료', source: '작품 정보 (시카고 미술관)', more: '같은 진열대의 다른 상자', back: '퍼즐 목록으로', startDaily: '시작', gridOf: (c: number, r: number) => `${c}×${r}` },
    play: { resume: '이어하기', resumeTitle: '하던 퍼즐', resumeHint: (pct: number, t: string) => `${pct}% 완성 · ${t}`, startOver: '새로 시작', saved: '저장됨', piles: '더미', pileAll: '전체', pileEdge: '테두리', pileNew: '+ 더미', pileName: (n: number) => `더미 ${n}`, sortColor: '색상순', shuffle: '섞기', outline: '조각 윤곽선', dropHere: '여기에 놓기', toPile: (n: string) => `${n}(으)로 옮김`, streak: (n: number) => `${n}일 연속 완주`, past: '지난 오늘의 퍼즐', done: '완성', notYet: '아직', today: '오늘' },
    guide: '가이드',
    about: { title: 'jigsawlab 소개', lead: '사진 한 장을 고르면 톱니가 맞물리는 진짜 직소 퍼즐이 됩니다. 회원가입 없이, 사진은 기기 밖으로 나가지 않고, 매일 하나의 퍼즐로 시간을 겨룹니다.', points: [['내 사진, 내 기기 안에서', '사진은 브라우저 안에서만 잘리고 그려집니다. 서버로 가지 않으니 가족 사진도 마음 놓고.'], ['매일 같은 판, 다른 기록', '오늘 방문한 모든 사람이 같은 컷을 맞춥니다. 자정에 새 그림이 걸리고 기록은 기기당 하나.'], ['폰에서 제대로', '두 손가락 확대와 트레이 끌어올리기까지, 손가락 하나로 끝까지 맞출 수 있게 만들었습니다.'], ['퍼블릭 도메인 명화 226점', '고흐·모네·르누아르·호쿠사이·김홍도·윌리엄 모리스. 상자를 열면 그림 이야기와 소장처가 있습니다.']], guide: '가이드' },
  ...
  en: {
    ...
    catalog: { title: 'Puzzle boxes', sub: 'Public-domain masterpieces, shelved like real boxes. Open one for the story behind the picture and pick your piece count.', all: (n: number) => `${n} puzzles`, search: 'Search title or artist', results: (n: number) => `${n} puzzles`, none: 'No matches', fav: 'Favourites', favAdd: 'Add to favourites', favRemove: 'Remove from favourites' },
  ...
  ja: {
    ...
    catalog: { title: 'パズルの箱', sub: 'パブリックドメインの名画を本物の箱のように並べました。箱を開くと絵の話とピース数を選べます。', all: (n: number) => `全${n}点`, search: '題名・作家で検索', results: (n: number) => `${n}点`, none: '該当なし', fav: 'お気に入り', favAdd: 'お気に入りに追加', favRemove: 'お気に入りから外す' },
    ...
    detail: { open: 'この絵で遊ぶ', pieces: 'ピース数を選ぶ', best: (t: string) => `自己ベスト ${t}`, noBest: 'まだ記録がありません', museum: '所蔵', year: '制作', medium: '素材', source: '作品情報（シカゴ美術館）', more: '同じ棚の他の箱', back: 'パズル一覧へ', startDaily: 'を始める', gridOf: (c: number, r: number) => `${c}×${r}` },
    play: { resume: '続きから', resumeTitle: '途中のパズル', resumeHint: (pct: number, t: string) => `${pct}% 完成 · ${t}`, startOver: '最初から', saved: '保存済み', piles: '山', pileAll: '全部', pileEdge: '縁', pileNew: '+ 山', pileName: (n: number) => `山 ${n}`, sortColor: '色順', shuffle: 'シャッフル', outline: 'ピースの輪郭', dropHere: 'ここに置く', toPile: (n: string) => `${n}へ移動`, streak: (n: number) => `${n}日連続完成`, past: '過去の今日のパズル', done: '完成', notYet: 'まだ', today: '今日' },
    guide: 'ガイド',
    about: { title: 'jigsawlab について', lead: '写真を一枚選ぶと、本物の凹凸がかみ合うジグソーパズルになります。登録不要、写真は端末の外に出ず、毎日ひとつのパズルでタイムを競います。', points: [['写真は端末の中だけ', '写真はブラウザの中で切り分けて描かれます。アップロードしないので家族写真も安心。'], ['毎日同じカット、自分の記録', '今日訪れた全員が同じカットを組みます。日本時間0時に新しい絵、記録は端末ごとに一つ。'], ['スマホで本気', '二本指ズームとトレイからのドラッグ。指一本で最後まで組めるように作りました。'], ['パブリックドメインの名画226点', 'ゴッホ・モネ・ルノワール・北斎・金弘道・ウィリアム・モリス。箱を開くと絵の話と所蔵先があります。']], guide: 'ガイド' },
