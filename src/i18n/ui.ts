...
export const UI = {
  ko: {
    ...
    nav: { daily: '오늘의 퍼즐', catalog: '퍼즐 상자', photo: '내 사진으로', play: '지금 맞추기', about: '소개', my: '내 퍼즐' },
    hero: { today: '오늘의 그림', pieces: (n: number) => `${n}조각`, solved: (n: number) => (n ? `오늘 ${n}명이 완성했어요` : ''), fastest: (t: string) => `가장 빠른 기록 ${t}`, cta: '오늘의 퍼즐 맞추기', photo: '내 사진으로 만들기', sameForAll: '', h1: '제대로 된 직소 퍼즐,\n폰에서도.', sub: '맞는 조각끼리 붙어서 뭉치로 움직이고, 색깔별로 더미를 나누고, 하던 판은 저장돼요. 판 위에 광고는 안 띄워요.', feats: ['맞는 조각끼리 붙어서 뭉치로', '색깔별로 더미 나누기', '하던 판은 그대로 저장', '48조각부터 1000조각까지', '내 사진으로도'], pick: '상자 고르기', todayLink: (t: string) => `오늘의 그림 · ${t} · 같이 맞추기` },
  ...
  en: {
    ...
    nav: { daily: "Today's puzzle", catalog: 'Puzzle boxes', photo: 'Your photo', play: 'Play now', about: 'About', my: 'My puzzles' },
    hero: { today: "Today's picture", pieces: (n: number) => `${n} pieces`, solved: (n: number) => (n ? `${n} people finished it today` : ''), fastest: (t: string) => `Fastest time ${t}`, cta: "Solve today's puzzle", photo: 'Make one from my photo', sameForAll: '', h1: 'A real jigsaw,\non your phone.', sub: 'Pieces that fit snap together and move as a cluster, sort into piles by colour, and your board is saved when you leave. No ads on the board, ever.', feats: ['Matching pieces snap into clusters', 'Sort into piles by colour', 'Your board is saved as you go', 'From 48 to 1000 pieces', 'Your own photos too'], pick: 'Pick a box', todayLink: (t: string) => `Today's picture · ${t} · solve it together` },
  ...
  ja: {
    ...
    nav: { daily: '今日のパズル', catalog: 'パズルの箱', photo: '自分の写真で', play: '今すぐ遊ぶ', about: 'サイトについて', my: 'マイパズル' },
    hero: { today: '今日の絵', pieces: (n: number) => `${n}ピース`, solved: (n: number) => (n ? `今日 ${n}人が完成` : ''), fastest: (t: string) => `最速タイム ${t}`, cta: '今日のパズルを解く', photo: '自分の写真で作る', sameForAll: '', h1: '本物のジグソーを、\nスマホでも。', sub: '合うピースはくっついて塊で動き、色ごとに山に分け、途中の盤面は保存されます。盤の上に広告は出しません。', feats: ['合うピースはくっついて塊に', '色ごとに山を分ける', '途中の盤面はそのまま保存', '48ピースから1000ピースまで', '自分の写真でも'], pick: '箱を選ぶ', todayLink: (t: string) => `今日の絵 · ${t} · 一緒に組む` },
