...
export const EN: UIStrings = {
  ...
  app: { install: 'Install app', installTitle: 'Install as an app', iosLead: 'Tap the Share button at the bottom of Safari and choose "Add to Home Screen" — it opens like an app from your home screen', iosStep1: 'Share button', iosStep2: 'Add to Home Screen', otherLead: 'This browser can\'t install directly. Open the site in Chrome, Edge, or Safari on iPhone to add it to your home screen as an app', samsungLead: 'Samsung Internet can\'t install this right now — Android rejects its package as "built for an older version" (a Samsung-side issue). Open the site in Chrome to install', openChrome: 'Open in Chrome' },
  hero: { today: "Today's picture", pieces: (n: number) => `${n} pieces`, cta: "Solve today's puzzle", photo: 'Make one from my photo', h1: 'A real jigsaw,\non your phone.', sub: 'Pieces that fit snap together and move as a cluster, sort into piles by colour, and your board is saved when you leave. Send a link and solve it with friends.', feats: ['Matching pieces snap into clusters', 'Sort into piles by colour', 'Your board is saved as you go', 'From 48 to 1000 pieces', 'Play with friends from one link', 'Your own photos too'], pick: 'Pick a puzzle', statPuzzles: 'puzzles', statSolved: 'boards finished', todayLink: (t: string) => `Today's picture · ${t} · solve it together` },
  catalog: { title: 'Puzzles', sub: '', cats: 'Categories', every: 'All', catH1: (name: string) => `${name} jigsaw puzzles`, catTitle: (name: string, n: number) => `${name} jigsaw puzzles — ${n} free online puzzles`, catDesc: (name: string, n: number) => `${n} pictures from ${name}, ready to solve as online jigsaw puzzles. Anything from 48 to 1000 pieces. Free, no sign-up, no install.`, showAll: (n: number) => `Show all ${n}`, prev: 'Previous', next: 'Next', all: (n: number) => `${n} puzzles`, search: 'Search title or artist', results: (n: number) => `${n} puzzles`, none: 'No matches',  noneAll: 'Search all puzzles →', fav: 'Favourites', favAdd: 'Add to favourites', undoneOnly: 'Unsolved only', progress: (a: number, b: number) => `${a} / ${b} solved`, doneMark: 'Solved', more: (n: number) => `Show ${n} more`},
  photo: {
  ...
  my: { title: 'My puzzles', inProgress: 'In progress', done: 'Finished', favs: 'Favourites', emptyProgress: 'Nothing in progress', emptyDone: 'No finished puzzles yet', emptyFavs: 'Tap the heart on a puzzle to collect it here', resume: 'Continue', del: 'Delete', pieces: (n: number) => `${n} pieces`, photo: 'My photo', doneCount: (n: number) => `${n} puzzles`, view: 'View', replay: 'Replay in the order you placed them', again: 'Solve again', timelapse: 'Timelapse', notFound: "Can't find that record", zoomHint: 'Scroll or pinch to zoom, drag to pan', carving: 'Carving the pieces…', photoDlg: 'No finished view', photoDlgLead: 'Photo puzzles never leave your device — the picture is erased when you finish, so there is nothing left to show.', onDevice: 'Stored on this device only' },
  lv: {
    ...
    rules: ['Every piece you place earns XP, and bigger boards pay more per piece — 1 up to 100 pieces, 1.5 at 200–300, 2 at 500, 3 at 1000, 4 at 2000. A finished 1000-piece board is 3,000 XP', 'Daily puzzles count 1.5×', 'Re-solving a picture you already cleared at that size counts a quarter', 'In a room you earn XP for the pieces you placed yourself', 'Up to 10,000 XP a day', 'Time and moves are not counted — the board rewards solving a lot, not solving fast', 'Days and weeks follow Korea Standard Time (KST, UTC+9). The weekly board resets Monday at midnight KST'],
    ...
    nextIn: (n: number) => `${n.toLocaleString()} XP to the next level`, maxed: 'Top level reached',
    ...
    gained: (n: number) => `+${n.toLocaleString()} XP`, levelUp: (n: number) => `Level ${n}!`, capped: "You've hit today's XP cap",
    ...
    earn: (n: string) => `+${n} XP when you finish`, earnRepeat: (n: string) => `+${n} XP when you finish · a quarter, since you already cleared this one`, earnDaily: (n: string) => `+${n} XP when you finish · daily counts 1.5×`, earnLive: (r: number) => `${r} XP for every piece you place`,
  },
  guide: 'Guide',
  ...
  foot: { privacy: 'Privacy', terms: 'Terms', termsTitle: 'Terms of Service', contact: 'Contact', rights: 'All paintings are in the public domain', copied: 'Link copied', shareFail: "Couldn't share", privacyTitle: 'Privacy policy', toHome: 'Home' },
};
