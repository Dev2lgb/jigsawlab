...
export const EN: UIStrings = {
  ...
  catalog: { title: 'Puzzles', sub: '', cats: 'Categories', every: 'All', catH1: (name: string) => `${name} jigsaw puzzles`, catTitle: (name: string, n: number) => `${name} jigsaw puzzles — ${n} free online puzzles`, catDesc: (name: string, n: number) => `${n} pictures from ${name}, ready to solve as online jigsaw puzzles. Anything from 48 to 1000 pieces. Free, no sign-up, no install.`, showAll: (n: number) => `Show all ${n}`, prev: 'Previous', next: 'Next', all: (n: number) => `${n} puzzles`, search: 'Search title or artist', results: (n: number) => `${n} puzzles`, none: 'No matches',  noneAll: 'Search all puzzles →', fav: 'Favourites', favAdd: 'Add to favourites', more: (n: number) => `Show ${n} more`},
  photo: {
