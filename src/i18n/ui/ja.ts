...
export const JA: UIStrings = {
  ...
  my: { title: 'マイパズル', inProgress: '途中のパズル', done: '完成したパズル', favs: 'お気に入り', emptyProgress: '途中のパズルはありません', emptyDone: 'まだ完成したパズルはありません', emptyFavs: 'パズルのハートを押すとここに集まります', resume: '続きから', del: '削除', pieces: (n: number) => `${n}ピース`, photo: '自分の写真', doneCount: (n: number) => `${n}枚`, stats: '統計', statTime: '合計時間', statPpm: '1分あたりのピース', statBiggest: '最大の盤', statDaily: '今日のパズル', byShelf: '棚ごとの進み', calendar: '完成した日', calN: (n: number) => `${n}枚`, hm: (h: number, m: number) => (h ? `${h}時間${m}分` : `${m}分`), emptyStats: '1枚完成すると統計が貯まります', view: '完成を見る', replay: 'はめた順に再生', again: 'もう一度組む', timelapse: 'タイムラプス', notFound: 'その記録が見つかりません', zoomHint: 'ホイールやピンチで拡大、ドラッグで移動', carving: 'ピースを刻んでいます…', onDevice: 'この端末にのみ保存されます' },
  lv: {
