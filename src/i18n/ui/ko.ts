...
export const KO = {
  name: 'jigsawlab', altName: '직소랩', tagline: '직소랩 — 온라인 직소 퍼즐. 명화와 내 사진으로, 폰에서도.', langName: '한국어', langGo: '한국어로 보기', langStay: '닫기', seoTitle: '직소랩(jigsawlab) — 온라인 직소 퍼즐, 명화와 내 사진으로', seoDesc: (n: number) => `직소랩은 명화·사진 ${n}점과 내 사진으로 맞추는 무료 온라인 직소 퍼즐이에요. 링크 하나로 친구를 불러 같이 맞추고, 맞는 조각끼리 붙고, 하던 판은 저장돼요. 48조각부터 1000조각까지, 가입 없이 폰에서도.`,
  nav: { daily: '오늘의 퍼즐', catalog: '퍼즐', together: '모두의 퍼즐', photo: '내 사진으로', play: '지금 맞추기', about: '소개', my: '내 퍼즐', rank: '랭킹' },
  app: { install: '앱 설치', installTitle: '앱으로 설치', iosLead: 'Safari 아래쪽의 공유 버튼을 누르고 "홈 화면에 추가"를 고르면 홈 화면에서 앱처럼 열려요', iosStep1: '공유 버튼', iosStep2: '홈 화면에 추가', otherLead: '이 브라우저는 바로 설치를 지원하지 않아요. Chrome·Edge 나 iPhone 의 Safari 에서 열면 홈 화면에 앱으로 추가할 수 있어요', samsungLead: '삼성 인터넷은 지금 앱 설치가 막혀 있어요 — 안드로이드가 "이전 버전용 앱"이라며 거부해요(삼성 쪽 문제). Chrome 에서 열어 설치해 주세요', openChrome: 'Chrome 에서 열기' },
  hero: { today: '오늘의 그림', pieces: (n: number) => `${n}조각`, cta: '오늘의 퍼즐 맞추기', photo: '내 사진으로 만들기', h1: '제대로 된 직소 퍼즐,\n폰에서도.', sub: '맞는 조각끼리 붙어서 뭉치로 움직이고, 색깔별로 더미를 나누고, 하던 판은 저장돼요. 링크 하나면 친구랑 같이 맞출 수 있어요.', feats: ['맞는 조각끼리 붙어서 뭉치로', '색깔별로 더미 나누기', '하던 판은 그대로 저장', '48조각부터 1000조각까지', '링크 하나로 친구랑 같이', '내 사진으로도'], pick: '퍼즐 고르기', statPuzzles: '퍼즐', statSolved: '완성된 판', todayLink: (t: string) => `오늘의 그림 · ${t} · 같이 맞추기` },
  catalog: { title: '퍼즐', sub: '', cats: '카테고리', every: '전체', catH1: (name: string) => `${name} 퍼즐`, catTitle: (name: string, n: number) => `${name} 퍼즐 ${n}점 — 무료 온라인 직소 퍼즐`, catDesc: (name: string, n: number) => `${name} 그림 ${n}점을 온라인 직소 퍼즐로. 48조각부터 1000조각까지 원하는 만큼 잘라 맞춰요. 가입도 설치도 없이 무료.`, showAll: (n: number) => `${n}점 전부 보기`, prev: '이전', next: '다음', all: (n: number) => `모두 ${n}점`, search: '제목·작가 검색', results: (n: number) => `${n}점`, none: '검색 결과가 없어요', noneAll: '전체에서 찾아보기 →', fav: '즐겨찾기', favAdd: '즐겨찾기에 추가', undoneOnly: '안 해 본 그림만', progress: (a: number, b: number) => `${a} / ${b} 완성`, doneMark: '완성', more: (n: number) => `${n}점 더 보기`},
  ...
  lv: {
    ...
    gained: (n: number) => `+${n.toLocaleString()} XP`, levelUp: (n: number) => `Lv ${n} 달성!`, capped: '오늘 XP 한도까지 채웠어요',
    seeRank: '랭킹', seeMy: '내 업적', signInGain: '로그인하면 다음 판부터 XP 가 쌓여요',
    ...
    earn: (n: string) => `다 맞추면 +${n} XP`, earnRepeat: (n: string) => `다 맞추면 +${n} XP · 이미 깬 그림이라 ¼`, earnDaily: (n: string) => `다 맞추면 +${n} XP · 오늘의 퍼즐은 1.5배`, earnLive: (r: number) => `내가 놓은 조각 하나에 ${r} XP`,
  },
  guide: '가이드',
  ...
  foot: { privacy: '개인정보처리방침', terms: '이용약관', termsTitle: '이용약관', contact: '문의', rights: '그림은 모두 퍼블릭 도메인입니다', copied: '링크를 복사했어요', shareFail: '공유가 안 됐어요', privacyTitle: '개인정보처리방침', toHome: '홈으로' },
};
