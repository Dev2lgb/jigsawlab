...
export function askNick(initial = '', mode: 'new' | 'edit' = 'edit'): Promise<string | null> {
  return new Promise((res) => {
    ...
    nickResolve?.(null); nickResolve = res; d.dataset.mode = mode; inp.value = initial || randomNick(LANG()); ($('nick-ok') as HTMLButtonElement).textContent = mode === 'new' ? d.dataset.start! : d.dataset.save!;
    show('nick-dlg', true); setTimeout(() => { inp.focus(); inp.select(); }, 40);
  });
}
function endNick(v: string | null) { const d = $('nick-dlg'), inp = $('nick-input') as HTMLInputElement | null; if (v === null && d?.dataset.mode === 'new') v = (inp?.value ?? '').trim().slice(0, 12) || randomNick(LANG()); show('nick-dlg', false); const r = nickResolve; nickResolve = null; r?.(v); } // 처음 정할 때 취소하면 제안된 이름으로
export function initAuthUI() {
  ...
  $('nick-cancel')?.addEventListener('click', () => endNick(null)); $('nick-shuffle')?.addEventListener('click', () => { if (inp) { inp.value = randomNick(LANG()); inp.focus(); } }); $('nick-bg')?.addEventListener('click', () => endNick(null));
  ...
  (window as any).__authui = { openLogin, askNick, randomNick: () => randomNick(LANG()) };
}
