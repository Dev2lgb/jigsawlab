...
import { clipNick, nickCells, NICK_CELLS } from './nick';
...
export function askNick(initial = '', mode: 'new' | 'edit' = 'edit'): Promise<string | null> {
  return new Promise((res) => {
    const d = $('nick-dlg'); const inp = $('nick-input') as HTMLInputElement | null; if (!d || !inp) { res(clipNick(prompt(d?.dataset.title ?? '', initial) || '') || null); return; }
...
function endNick(v: string | null) { const d = $('nick-dlg'), inp = $('nick-input') as HTMLInputElement | null; if (v === null && d?.dataset.mode === 'new') v = clipNick(inp?.value ?? '') || randomNick(docLang()); show('nick-dlg', false); const r = nickResolve; nickResolve = null; r?.(v); } // 처음 정할 때 취소하면 제안된 이름으로
export function initAuthUI() {
  ...
  form?.addEventListener('submit', (e) => { e.preventDefault(); const n = clipNick(inp?.value ?? ''); if (!n) { inp?.focus(); return; } endNick(n); });
  inp?.addEventListener('input', () => { if (nickCells(inp.value) > NICK_CELLS) inp.value = clipNick(inp.value); }); // 폭으로 자른다 — maxlength 는 글자 수라 한글 16자가 들어온다
