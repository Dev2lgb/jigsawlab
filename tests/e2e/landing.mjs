...
{ const { ctx, page } = await newPage(br);
  ...
  await ctx.close(); }
// 랭킹의 네임태그 견본 — 정적으로는 이름·글자만(색인), 스크립트가 판 일곱 가지(판지 0 … 흑요석 6)를 그린다. 비회원은 견본 이름·내 명판 표시 없음,
...
{ const { ctx, page } = await newPage(br); await page.goto(BASE + '/rank/', { waitUntil: 'networkidle' }); await page.waitForSelector('#rk-plates .ntag[data-plate]', { timeout: 15000 });
  ...
  ok(g.map((x) => x.p).join() === '0,1,2,3,4,5,6' && g.every((x) => x.t === '퍼즐러' && !x.mine), 'rank plates(비회원): 판 일곱 가지, 견본 이름, 내 명판 없음', JSON.stringify(g));
  await ctx.close(); }
